...
function Validator(options) {
    ...
    function validate(inputElement, rule) {
        ...
        const errorElement = parentInputElement.querySelector(options.errorSelector)
        ...
        formElement.onsubmit = function(e) {
            ...
            if(isFormValid) {
                ...
                if(typeof options.onSubmit === 'function') {
                    ...
                    const formInputValues = Array.from(enableInputs).reduce((values, input) => {
                        switch(input.type) {
                            case 'radio':
...
Validator.minLength = function(selector, min, message) {
    return {
        selector,
        test(value) {
            return value.length >= min ? undefined : message || `Trường này tối thiểu ${min} ký tự`
        }
    }    
}
